...
#include "matemat/geom-2.0/src/listas/GmGrupo.h"
...
inline forma_medicion str2forma_medicion(const std::string &str)
{
...
public:
    ...
    Pieza(const UdObra &u, const forma_medicion &f): ProtoPartida(u), fm(f) {}
    virtual ProtoPartida *Copia(void) const
    {
        return new Pieza(*this);
    }
    long double Total() const
    {
        return Meds().Total();
    }
    ppl_dimension TotalR() const
    {
        return Meds().TotalR();
    }
    Mediciones Meds(void) const
    {
        ...
        metodo_medida mm= MetodoMedida();
        if(mm)
        {
            GmGrupo::const_iterator i= geom.begin();
            for(; i!=geom.end(); i++)
